...
public:
	...
	Matrix(int _row, int _col, ForwardIter begin, ForwardIter end){this->set(_row, _col, begin, end);};
	...
	template<typename ForwardIter>
	void set(int _row, int _col, ForwardIter begin, ForwardIter end);
...
template<typename T>
template<typename ForwardIter>
void Matrix<T>::set(int _row, int _col, ForwardIter begin, ForwardIter end) {
	if(_row*_col <= 0) {
		msgError(NOPOSITIVE);exit(0);//underthinking
	}
	this->row = _row; this->col = _col;this->mat.resize(_row*_col);
